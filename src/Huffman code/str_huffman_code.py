...
def build_codes(node, current_сode = "", codes = None):
    if codes is None:
        codes = {}
    ...
    if node.char is not None:
        codes[node.char] = current_сode
        return
    
    build_codes(node.left, current_сode + "0", codes)
    build_codes(node.right, current_сode + "1", codes)
...
msg = "abracadabra"
code, table = encode(msg)
print(code, table)
print(decode(code, table))
